...
wget https://github.com/FFmpeg/FFmpeg/archive/n7.1.1.tar.gz
...
if [ "$ARCH" = x86_64 ]; then
	...
	if [ ! -f "cache/wgcf" ]; then
		func_curl --output wgcf "https://github.com/ViRb3/wgcf/releases/latest/download/wgcf_2.2.24_linux_amd64"
		chmod +x "cache/wgcf"
	fi
	...
	if [ ! -f "cache/wgcf" ]; then
		func_curl --output wgcf "https://github.com/ViRb3/wgcf/releases/latest/download/wgcf_2.2.24_linux_arm64"
		chmod +x "cache/wgcf"
	fi
	if [ ! -f "cache/yt-dlp" ]; then
